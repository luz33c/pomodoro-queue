...
async function shouldEnforce() {
  const s = (await storage.get<PomodoroState>(STORAGE_KEY)) as
    | PomodoroState
    | undefined
  return Boolean(
    s?.config?.strictMode &&
      !s?.paused &&
      (s?.phase === 'short' || s?.phase === 'long')
  )
}
...
export function initStrictBreakKernel() {
  ...
  chrome.tabs.onUpdated.addListener(async (tabId, change, tab) => {
    ...
    if (
      breakTabIdsByWindow.has(tab.windowId!) &&
      breakTabIdsByWindow.get(tab.windowId!) === tabId
    ) {
