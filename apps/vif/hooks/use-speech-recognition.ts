...
export function useSpeechRecognition() {
  ...
  const startRecording = async () => {
    ...
    try {
      ...
      for (const type of mimeTypes) {
        if (!type || MediaRecorder.isTypeSupported(type)) {
          try {
            ...
            console.warn(
              `Failed to create recorder with mime type ${type}:`,
              e
            );
          }
        }
      ...
      if (!recorder) {
        throw new Error(
          'Could not create MediaRecorder with any supported format'
        );
      }
  ...
  const processSpeechToText = async (
    ...
    try {
      ...
      alert(
        "Sorry, we couldn't convert your speech to text. Please try again."
      );
      return null;
    } finally {
      setIsProcessingSpeech(false);
